...
from utils.manifest_utils import write_manifest
import logging
...
def generate_samples(num_samples: int, max_seq_length: int, save_dir: str, incremental: int = 10):
    ...
    print(f"Max sequence length after model template token adjustment: {max_seq_length}")
    # Estimate tokens per question to determine reasonable upper bound
    sample_input_text, _ = generate_input_output(0, incremental)
    print(f"Sample input text: ...")  # Print first 100 chars for debugging
    ...
    for index in tqdm(range(num_samples)):
        ...
        if length > max_seq_length:
            logger.warning(f"Generated length {length} exceeds max_seq_length {max_seq_length} and used_docs {used_docs}.")
...
def main():
    print(f"Generating {args.num_samples} samples with max sequence length {args.max_seq_length} and tokens to generate {args.tokens_to_generate}.")
